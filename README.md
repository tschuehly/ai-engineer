...
Build a local index of recent videos from the AI Engineer YouTube channel, then use `gnhf` to analyze one transcript per iteration into a progressive-disclosure Markdown knowledge base.
...
Each iteration analyzes exactly one previously unprocessed transcript from `data/ai_engineer_youtube/index.jsonl` and updates:

- `data/ai_engineer_youtube/knowledge_base.md`

Run more iterations when you want the knowledge base to grow:

```bash
cat prompts/gnhf-build-knowledge-base.md | gnhf --agent codex --max-iterations 10
```

The Markdown file contains a concept map, atomic source-backed knowledge entries, and a processed-source log.
