...
- **The other half of the same discipline, which has no trigger.** This page's procedure starts from a score that *moved* — a drop-and-hold is what prompts someone to read traces. Fox documents the failure mode with no such prompt: a serious judge that under-calls, passing one in five notes that hid a serious error, "not a safety net… a second silent failure that just nods along with the first." Nothing on a dashboard moves, no alert fires, and the artifact produced is a clean pass. The detection method is therefore different in kind rather than in degree — over-calling is found by reading what the judge *flagged*, under-calling only by expert-reviewing a sample of what it *passed*, on a schedule rather than on a signal. Both belong in the same standing obligation. See [A Judge Without Taste Is a Second Silent Failure](a-judge-without-taste-is-a-second-silent-failure.md). ([Fox](../sources/20260822_yqF6XhzbWBk.md), 09:24-11:20)
...
- [Inside 847 Production Clinical AI Notes — Sebastian Fox, Composo](../sources/20260822_yqF6XhzbWBk.md), 09:24-11:20
