...
- **The constraint on hint content that this page does not supply.** Specificity is the active ingredient here, and the obvious way to maximize it — put the resolution in the hint — is the failure mode a second vendor names as the analogue of reward hacking: a hint containing the answer trains a model that states the answer and back-fills the reasoning, which "is not going to occur whatsoever in the real world" ([Hint Leakage Is the Reward Hacking of Self-Distillation](hint-leakage-is-the-reward-hacking-of-self-distillation.md)). Read the two together as the two halves of hint design: rollout-specific enough to name the actual error, restrained enough to say what the model should have *noticed* rather than what it should have *concluded*.
