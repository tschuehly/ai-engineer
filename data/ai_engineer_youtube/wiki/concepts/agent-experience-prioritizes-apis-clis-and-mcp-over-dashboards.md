...
- **The agent surface is also a recommendation channel, which changes who pays for it.** Jarmak describes the agent as occupying both sides at once — "it is both the user of your tool in a very similar way to the developer… going out reading your docs… calling the API," and "it's also a recommender of your tools," now driving the bottom-up adoption that developer relations was built to reach. That makes API/CLI/MCP investment a distribution line item rather than only a usability one, and it supplies the argument for funding it: "by serving the agents, the human path gets cleared, too." See [Treat Agent Experience as a Curb Cut](treat-agent-experience-as-a-curb-cut.md). ([Jarmak](../sources/20260826_Lrw0jqBNaw0.md), 04:15-05:22, 16:45-17:21)
- **The case where the product behind the API is itself an agent platform.** Warp's rule is "an API for every key component of the stack… for spinning up agents and subagents, for managing the environments and compute that these agents are running in, for working with the artifacts that they produce," justified by composability: users are then "not restricted to your UI or your opinion of how a particular experience should look." The consumer here is neither an agent browsing your product nor a human clicking a dashboard — it is a script a non-engineer wrote to *start* agent work from an external event, which is what Warp's developer-relations team built for social mentions. The three primitives (the run, the place it runs, the output) generalize as a coverage test for any agent product's API surface. See [Expose Orchestration Through Both a Prompt and an API](expose-orchestration-through-both-a-prompt-and-an-api.md). ([Abdalla](../sources/20260822_L173Z8DpaJg.md), 08:29-10:06)
...
- [The Death of Developer Advocates — Stephanie Jarmak, Sourcegraph](../sources/20260826_Lrw0jqBNaw0.md), 04:15-05:22, 16:45-17:21
- [The Agent Behind the Curtain: Building the Oz Cloud Agent Platform — Safia Abdalla, Warp](../sources/20260822_L173Z8DpaJg.md), 08:29-10:06
