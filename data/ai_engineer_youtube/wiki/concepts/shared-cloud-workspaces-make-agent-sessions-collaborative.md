...
- **A later status report on the same prototype, from the lab's lead, adds the selection criterion the design was missing.** Idan Gazit restates the substrate in one line — "every one of these is not on my machine. It's all micro VMs in the cloud. So every session is just a branch of my repo checked out to a spot in the cloud" — and concedes the obvious objection to a chat-shaped IDE before making the case: "Slack was designed to be better than email for the average office worker. It was never designed for making software." What justifies the shape is what the surface carries, not the surface itself: "anything that's in code, any fact that's in code, the agents can figure out by reading the code. What's left are the things that are not in code" ([tell the agent only what is not recoverable from the code](tell-the-agent-only-what-is-not-recoverable-from-the-code.md)). He also separates two requirements this page treats as one: real-time multiplayer is not sufficient, because "I also want to be ambiently aware of what everybody's going on about," which is a different feature from a shared session. ACE was still pre-release at this talk — "technical preview hopefully later this month." ([Idan Gazit](../sources/20260808_iQ5xldZ9StU.md), 13:48-14:30, 14:51-15:17, 18:37-19:02, 20:58-21:14)
...
- [Realtime multiplayer, automation, and you! — Idan Gazit, GitHub](../sources/20260808_iQ5xldZ9StU.md), 13:48-15:17, 18:37-19:02, 20:58-21:14
