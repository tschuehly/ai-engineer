...
- The video case, from a different team and a different substrate: Reelful's agent authors a Remotion composition — video as React code, "a file with the order with all your assets and tracks and how they're following each other" — and the MP4 the consumer receives is a downstream render of it, with a verification layer between the two that rejects compositions that will not render ([Reelful](../sources/20260818_pPj_tjlvYjA.md), 07:12-08:19). Same split: the editing format is the one agents write well, the delivery format is whatever the audience consumes.
