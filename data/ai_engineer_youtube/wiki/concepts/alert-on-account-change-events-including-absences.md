...
- **Two signal families held in the same layer.** A go-to-market data platform ingests "buying signals" of two kinds side by side: internally modeled ones — "we think that this customer has a high propensity to attach to procurement or treasury" — and external ones such as funding announcements, alongside raw interaction data (emails, meetings, calls, page views). Keeping a model output and an external event in the same signal namespace is what lets one downstream consumer act on both, but it also means a signal's reliability now varies by kind, and this source gives no way to tell them apart at read time. ([Vaziri](../sources/20260826_VjEP0xqTUI0.md), 05:55-06:28)
...
- [The Building Blocks of GTM Orchestration — Arman Vaziri, Ramp](../sources/20260826_VjEP0xqTUI0.md), 05:55-06:28
