...
- **How to pick that one skill without asking anyone.** If an unwritten skill shows up as spend rather than as a request, the highest-value candidate is derivable from the bill: sort recurring task classes by cost per outcome and start at the top, because that is where engineers are re-deriving the same constraints alone. Touil's regulation example is the archetype — nothing fails, so nothing is filed, and the cost surfaces only as steering iterations and tokens. ([Touil](../sources/20260828_M05vON8i0aI.md), 16:18-16:42) It also predicts which team is worth watching: the one that looks average on productivity and quality "but when it came to the cost is really high" (16:57-17:15). Illustrated with a self-authored simulation, not measured.
...
- [AI-Native Organisations Run on Skills: How to Structure and Scale Them — Imad Touil, QuantumBlack](../sources/20260828_M05vON8i0aI.md), 16:18-17:15
