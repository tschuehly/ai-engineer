...
- **A stronger version of the same conclusion that does not need the exhaustion premise.** Jack Morris, speaking at the same event, argues that the purchased post-training layer is public *by definition* — expert annotation from "data acquisition companies like Scale AI, Surge AI, and Mercor" is "still by definition creating publicly available data because it's something that the model could tell to a user." On that argument the private axis stays untouched no matter how much annotation budget exists, so the case for private worlds does not depend on the contested claim that public data is used up ([Post-Training Data Is Public by Construction](post-training-data-is-public-by-construction.md)). It also names the tension on this page more precisely: "channel back the learning to the general model" would make one customer's knowledge disclosable to every other, which is exactly the property that defines data out of privacy in Morris's account. ([Engram](../sources/20260812_WiqDvX6isc4.md), 05:47-06:04)
...
- [Scaling Compute on Context — Jack Morris, Engram](../sources/20260812_WiqDvX6isc4.md), 05:47-06:04
