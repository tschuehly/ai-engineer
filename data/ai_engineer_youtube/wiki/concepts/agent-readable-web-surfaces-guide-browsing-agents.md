...
- **Three implementation corrections from a team that shipped all of it.** First, do not generate the map: "it's much better not to just generate this. It is much better to write your `llms.txt` from hand… For about 40 good lines beats 1,000 lines of noise from our testing" ([Hand-Write llms.txt and Index the Rest for Fetching](hand-write-llms-txt-and-index-the-rest-for-fetching.md)). Second, note a naming conflict with the entry above: this page records `llms-full.txt` as bringing a site's content into a single file, while Burns describes the full form as a link index with a one-line purpose per page, on the grounds that "agents don't know how to browse. They know how to fetch." Those are opposite artifacts — one maximizes context, the other minimizes it — so state which one you publish. Third, ship the Markdown through more than one route, because "not all agents can append header tags" ([Serve Markdown Through Three Redundant Paths](serve-markdown-through-three-redundant-paths.md)). ([Burns](../sources/20260826_V_5bn4q-vAI.md), 05:52-09:06)
- **The scope limit that matters most for library vendors: the agent may never reach any of these surfaces.** "Coding agents are actually never visiting the website if you have a library. They're actually visiting the node modules. They read the repo and they read the node modules." Every surface on this page is published on the web and assumes a fetch; for an installable module the higher-leverage location is inside the published package. See [Ship Bundled Docs and an AGENTS.md Inside the Published Package](ship-bundled-docs-and-an-agents-md-inside-the-published-package.md). ([Burns](../sources/20260826_V_5bn4q-vAI.md), 10:04-11:16)
...
- [How We Got LLMs to Recommend Our Open Source Library — Christopher Burns, Inth](../sources/20260826_V_5bn4q-vAI.md), 05:52-09:06, 10:04-11:16
