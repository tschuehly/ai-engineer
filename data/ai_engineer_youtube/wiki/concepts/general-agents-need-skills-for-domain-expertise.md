...
- **The domain can be one company's own data model, and the skill file then has a second consumer.** Cloudflare's skill files are scoped by role rather than by profession, carrying business semantics for a specific warehouse; the same files are reused to build internal applications — "usually that is done in IT and bottlenecked in those areas, we're able to use the semantic information about the business knowledge, as well as the columns table to build these applications rather quickly." A skill written so a general agent can answer questions turns out to be the specification a coding agent needs to generate software, which is an argument for authoring domain context in a form that is not chat-specific. ([Joyce](../sources/20260826_Qw_tC68KKes.md), 06:47-08:50)
...
- [How AI Agents Let GTM Teams Scale — Justin Joyce, Cloudflare](../sources/20260826_Qw_tC68KKes.md), 06:47-08:50
