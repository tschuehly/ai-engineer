...
- **The tier list is treated as settled at the hardware-research level.** Arora names the same spread as a given when motivating multi-GPU kernel work: "KV cache memory in modern inference systems is going to be forked across GPU, CPU, disk and remote machines." The "remote machines" tier is the one this page's load tests most often omit, and it is the one whose throughput depends on the interconnect rather than on the storage device — which is why it needs to be measured on the fabric you actually have. ([Arora](../sources/20260827_pOvWgX7IJsc.md), 08:19-08:47)
...
- [Can LLMs Write Fast Multi-GPU Kernels? — Simran Arora, Together AI](../sources/20260827_pOvWgX7IJsc.md), 08:19-08:47
