# Make the Running Prototype the Primary Design Artifact

Summary: With coding agents, a designer can iterate directly on a running coded prototype and demote the design tool (Figma) to late visual fine-tuning; owning the whole plan-build-deploy loop instead of just designing collapses handoff and turns a designer into a "design engineer."

Use when:
- Reshaping a design workflow around AI, or deciding whether to keep building high-fidelity mockups versus prototyping in code.
- Explaining how a non-engineer can own a product end to end once dev-environment and enablement scaffolding exists.

Details:
- Old process: start in Figma and stay there until very high fidelity, then hand off. New process: a good plan → build the prototype/product in code (the live product is what you iterate on directly) → return to Figma only later to build a mood board and do UI fine-tuning; use Figma to hand the agent an image when something can't be explained in words. (15:20-16:07)
