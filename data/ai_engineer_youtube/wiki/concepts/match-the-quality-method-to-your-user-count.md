...
- **The same question asked about code quality rather than eval method produces a third axis: adversariality.** Chatterjee (Sonar) sizes the tolerable gap between default AI code quality and required quality the same way — for "a short-lived project that's not going to last very long," or an internal non-critical app with a few users, "the gap… is quite small… and you can live with that gap," while at higher criticality it is not — but his high end is defined by three things at once: many users, "a larger code base with many lines of code and many changes happening across that code base all the time," and users who "could be adversaries that are actively trying to break your software." The last one does not follow from either axis on this page. A five-user internal app with an untrusted input path is low-volume, high-consequence, *and* adversarial, and adversariality is what decides whether a security layer is optional. See [Verification Debt Outlives the Productivity Spike](verification-debt-outlives-the-productivity-spike.md). ([Chatterjee](../sources/20260809_03l29gJXpCE.md), 02:46-03:53)
...
- **A data point in the middle range this page leaves unaddressed, and what is available there.** Snowflake's internal assistant sits between the two poles: ~6,000 users and ~40,000 questions a week — far past hand review of every trace, far short of the free-tier experiment slice. The methods Izmit reports are all of the read-the-population kind rather than the compare-two-arms kind: a hand-written 150-question set derived from the business process for a pre-traffic baseline, LLM classification of the whole log into a topic taxonomy for demand, behavioral negative labels (repeated questions, profanity) for quality, and a weekly-active retention bar as the launch gate. No A/B test appears anywhere in the talk. That suggests the middle is not a degraded version of either pole but its own regime, where census methods over the full log substitute for both sampling and statistics. ([Building GTM AI Agents: Lessons from Deploying to 6,000 Users — Sait Izmit, Snowflake](../sources/20260826_DrTdD-ttjCY.md), 04:08-04:42, 06:41-06:58, 14:11-15:20)
...
- [Guide, Verify, Solve — Anirban Chatterjee, Sonar](../sources/20260809_03l29gJXpCE.md), 02:46-03:53
- [Building GTM AI Agents: Lessons from Deploying to 6,000 Users — Sait Izmit, Snowflake](../sources/20260826_DrTdD-ttjCY.md), 04:08-04:42, 06:41-06:58, 14:11-15:20
