...
- **A marketplace can only deliver capacity in a shape the workload will accept, which makes the workload's shape the other half of the problem.** Modal's RL talk states the limit for the hardest case: the default RL loop "asks one tightly coupled cluster. And that cluster is exactly the hard part to get," and it wants four properties at once — enough GPU, same region, fast fabric, available now ([RL Post-Training Demands Four Scarce Compute Properties at Once](rl-post-training-demands-four-scarce-compute-properties-at-once.md)). No aggregation layer resolves that, because the fragmentation the marketplace is pooling is exactly what the workload refuses. ([Modal](../sources/20260810_maRzp4kImJ4.md), 02:06-02:56)
- The complementary move is to change the workload so the pooled supply becomes usable — split the rollout fleet from the trainer, sync weights by patch, and let engines at any provider self-serve versions from a shared board, at which point "scattered inference capacity became one elastic rollout fleet." Read together, the two pages say aggregation and decoupling are the same project attacked from opposite ends. ([Modal](../sources/20260810_maRzp4kImJ4.md), 15:16-15:34, 18:17-18:43)
...
- [Taking Reinforcement Learning Cross Datacenter — Nan Jiang, Modal](../sources/20260810_maRzp4kImJ4.md), 02:06-02:56, 15:16-15:34, 18:17-18:43
