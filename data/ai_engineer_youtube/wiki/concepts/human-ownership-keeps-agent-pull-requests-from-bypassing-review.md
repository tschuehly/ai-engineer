...
- **Gating notification is not the same as gating merge, and the distinction is worth keeping.** Warp's open-source repo does not ping a human reviewer "until an agent has approved our PRs," so humans see "only the high-signal, high-quality ones" — yet humans still review and merge what surfaces. The ownership property this page defends survives, because the accountable human is downstream of the gate rather than removed by it. What does change is the set of changes a human ever *considers*: a PR the agent never approves does not appear in anyone's queue, which is a quieter failure than a bot-owned PR appearing in a queue with no owner. Any org adopting the pattern should be able to say what happens to the un-approved remainder. See [Put an Agent Approval Gate in Front of Maintainer Attention](put-an-agent-approval-gate-in-front-of-maintainer-attention.md). ([Abdalla](../sources/20260822_L173Z8DpaJg.md), 11:13-12:45)
...
- [The Agent Behind the Curtain: Building the Oz Cloud Agent Platform — Safia Abdalla, Warp](../sources/20260822_L173Z8DpaJg.md), 11:13-12:45
