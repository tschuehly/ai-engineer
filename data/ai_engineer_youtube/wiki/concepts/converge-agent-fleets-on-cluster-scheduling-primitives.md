...
- **The same convergence, one layer down, and with a caveat about how far the primitives reach.** llm-d is the inference-side instance of this argument: a CNCF-hosted, Kubernetes-native distributed inference framework that adds only the domain-specific pieces on top of the scheduler — a router with endpoint-picker plugins, and LeaderWorkerSet and DisaggregatedSet workload APIs "that orchestrate complex multi-node model execution" — rather than reinventing placement and scaling. The caveat is what the concurrency sweep shows: default Kubernetes scheduling is the *baseline* line, and adding domain-aware routing on top of it moves the curve visibly without changing the serving mode. Declaring what you need is necessary but not sufficient when the scheduler has no model of KV-cache locality. ([Kamra](../sources/20260827_YXowceUKYJI.md), 09:28-10:20, 13:52-14:37)
...
- [KV Cache-Aware Routing and P/D Disaggregation on Kubernetes — Yuchen Fama & Ashish Kamra, Red Hat](../sources/20260827_YXowceUKYJI.md), 09:28-10:20, 13:52-14:37
