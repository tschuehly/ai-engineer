...
- **The same decay in per-engineer steering files, with the model generations named and the re-test made a habit rather than an event.** "The Sonnet 3.7 in the middle of last year had a lot of quirks that we had to put a lot of do nots in our steering files, and now we don't have to do that as much with Opus 4.5 as of last November, and then we've had… more than 6 months of improvement since then. And so the question, the new habit, again, is do I still need this in my steering files or is this just bloating context?" Two things are worth taking. First, the cost named here is context occupancy rather than latency or cache — a stale "do not" is not an intervention that fires, it is tokens the model reads on every turn and may still obey. Second, Liguori pairs the pruning question with its opposite as one habit: the addition trigger is "every time the agent makes a mistake… what am I missing in my skills files?" and the subtraction trigger runs continuously alongside it, which is what keeps the file from being append-only between upgrades. Asserted from internal practice with no measurement of how much context was reclaimed. ([Liguori](../sources/20260828_pqlWNihgdjI.md), 08:47-09:39)
