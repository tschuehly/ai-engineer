...
- **A second place the network becomes the constraint, this one inside the serving cluster.** Prefill/decode disaggregation moves the KV cache between pods on every request — the decode worker "pulls the computed KV caches across the network fabric utilizing the KV transfer metadata" — which turns an interconnect into a hard prerequisite: "you must possess an advanced high-speed network fabric like RDMA or RoCE," and its absence is called "the biggest point" in favour of staying aggregated. Both cases share a shape worth naming: once a technique makes computation cheap enough, the remaining cost is moving state, and the fix is transport rather than more compute. ([Kamra](../sources/20260827_YXowceUKYJI.md), 11:56-12:55, 16:22-17:04)
- **The same shape one level down, in the hardware itself.** The compute-versus-transport divergence this page observes at the API boundary is also a silicon trend: A100 to B200 gave 7.2x more BF16 tensor core throughput against 3x intra-node and 2x inter-node communication, so "communication is increasingly consuming the majority of the runtime" on large distributed workloads. Three instances now share the shape — agent turns, disaggregated serving, and multi-GPU kernels — and in all three the fix is the same category of move: send less, or send it over a better path, rather than compute faster. ([Arora](../sources/20260827_pOvWgX7IJsc.md), 06:45-07:04, 10:30-10:46)
...
- [KV Cache-Aware Routing and P/D Disaggregation on Kubernetes — Yuchen Fama & Ashish Kamra, Red Hat](../sources/20260827_YXowceUKYJI.md), 11:56-12:55, 16:22-17:04
