...
- **A domain where the harness is currently load-bearing and cannot be thinned yet.** On multi-GPU kernel generation, adding a plain multi-turn loop with a bash environment took a frontier model from 24 to 35 of 87 problems — a large fraction of total capability supplied by the harness rather than the model. But the same experiment shows the thinning schedule is not automatic: the gain plateaued, and the remaining failures survive putting the missing principles directly in context. Thin the harness in response to measured model improvement on your task, not on the general trend line. ([Arora](../sources/20260827_pOvWgX7IJsc.md), 27:07-27:56, 29:11-29:17)
