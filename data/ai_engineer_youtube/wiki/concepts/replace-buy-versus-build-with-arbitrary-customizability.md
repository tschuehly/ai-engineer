...
- **A vendor's own version of the test, applied to the layer it sells.** Berry describes Clay's orchestration graph and then declines to make it a purchase argument — "whether you buy it or build it, I think this is like fundamentally the modern way to set this up" — which is the customizability criterion stated as a shape requirement instead of an access requirement: what matters is that the layer expose agents, tool calls, conditionals, code, and fan-out as composable primitives. A bought orchestrator with a fixed connector catalog and no code node fails that test whether or not it has an API. ([Berry](../sources/20260826_UhCY231d0FQ.md), 08:55-09:32)
