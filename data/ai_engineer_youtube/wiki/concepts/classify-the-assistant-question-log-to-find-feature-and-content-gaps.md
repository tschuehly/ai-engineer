...
- **The pre-traffic counterpart, and why it is a different instrument.** Cloudflare arrived at its question set "through testing" before there were logs to mine, and shipped it inside role-specific skill files. Log classification finds what people ask a system that already exists, and cannot see the questions nobody bothers to ask because the tool visibly cannot answer them; a written-up-front question set can, and cannot see the questions nobody anticipated. The two instruments have complementary blind spots, which argues for running the up-front set as the launch content and the log taxonomy as the refresh mechanism. ([Joyce](../sources/20260826_Qw_tC68KKes.md), 07:27-07:53)
