...
- **A coarser field-level signal that needs no confidence score: who wrote it.** Berry's agents write into their own CRM columns, kept "separat[e from] the fields that deterministic systems are updating or that people are updating." That gives every consumer a per-field provenance flag by construction, which is weaker than a calibrated confidence but available immediately and impossible to miscalibrate. The two compose: provenance says whether a value is worth scrutinizing at all, confidence says how much. ([Berry](../sources/20260826_UhCY231d0FQ.md), 12:23-12:43)
