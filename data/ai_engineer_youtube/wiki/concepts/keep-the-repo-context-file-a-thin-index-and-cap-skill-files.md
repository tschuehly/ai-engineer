...
- **The pattern crossing a package boundary.** The thin index need not be your own repo's. Burns publishes an `AGENTS.md` *inside* the npm package that "basically says, if you've got a problem, if you've got a question, all the documents are here. Grab them" — a pointer file authored by the dependency's maintainer, landing in the consumer's `node_modules`, doing exactly the job this page describes one layer further out. See [Ship Bundled Docs and an AGENTS.md Inside the Published Package](ship-bundled-docs-and-an-agents-md-inside-the-published-package.md). ([Burns](../sources/20260826_V_5bn4q-vAI.md), 10:46-12:10)
- **The same budget rule, restated for a different file with a number attached.** For `llms.txt` the finding is "about 40 good lines beats 1,000 lines of noise from our testing" — the identical claim that an orientation file's value is inverse to its length, arrived at independently for the web-facing surface rather than the repo-facing one. See [Hand-Write llms.txt and Index the Rest for Fetching](hand-write-llms-txt-and-index-the-rest-for-fetching.md). ([Burns](../sources/20260826_V_5bn4q-vAI.md), 06:14-06:39)
...
- [How We Got LLMs to Recommend Our Open Source Library — Christopher Burns, Inth](../sources/20260826_V_5bn4q-vAI.md), 06:14-06:39, 10:46-12:10
