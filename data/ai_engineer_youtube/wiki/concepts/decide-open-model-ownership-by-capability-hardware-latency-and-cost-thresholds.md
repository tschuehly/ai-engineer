...
- **The hardware threshold is usually evaluated against the wrong hardware.** Red Hat's GLM 5.2 case study starts from the gap between published numbers and installed base: "You probably have seen tons of impressive numbers of GLM 5.2 on B200. When we talk to our customers… they usually don't have the luxury of B200. They have a lot of H200." Their response is not to wait for the hardware but to re-derive the serving topology for it — a prefill pool of up to three throughput-optimized workers against one latency-optimized decode worker, NIXL for KV transfer, each worker a LeaderWorkerSet group at TP1/DP8/EP8 — reporting 4x faster TTFT and 60% more requests on a prefill-bound agentic dataset. The threshold this page describes as "fits the right hardware" is therefore not a property of the model alone; it is a property of the model plus a serving topology you are willing to design. ([Fama](../sources/20260827_YXowceUKYJI.md), 18:01-19:28, 19:52-20:20)
...
- [KV Cache-Aware Routing and P/D Disaggregation on Kubernetes — Yuchen Fama & Ashish Kamra, Red Hat](../sources/20260827_YXowceUKYJI.md), 18:01-19:28, 19:52-20:20
