...
- **Hybrid search used as a scoping mechanism, not just a relevance mechanism.** A go-to-market agent stack chunks and embeds call transcripts, emails, enablement materials, product knowledge, and playbooks into Turbopuffer, then runs "some combination of vector search, attribute search, keyword search in order to pull information scoped to a specific account, for example, without having to pull in the full raw corpus into agent context, which would also be very inefficient, very expensive." The attribute filter is what makes the account boundary enforceable rather than hoped for — semantic similarity alone will happily return a passage about a different customer, and in this setting that is a correctness failure before it is a relevance one. ([Vaziri](../sources/20260826_VjEP0xqTUI0.md), 11:16-12:09)
...
- [The Building Blocks of GTM Orchestration — Arman Vaziri, Ramp](../sources/20260826_VjEP0xqTUI0.md), 11:16-12:09
