...
- **A model builder reaches the same mix, and adds the reason the browser is a good place to run agent-written code.** Yutori's Navigator started as pure screenshot-in / clicks-out and the next version "can also write JavaScript on demand" inside the page — a Chrome-extension trace shows one `execute js` action filling several form fields at once "because it wrote a little bit of a function. It can read the code when necessary. It can write code when necessary." The argument for co-locating the two: "the browser after all is an engine that can execute code, but it has a sort of formal verification system built in. It is seeing the screenshot — that is the source of the truth. So it knows whether it succeeded or not." The screen is an independent check on code the model just wrote, which is [verification through a different channel than the one that acted](verify-an-action-through-a-different-channel-than-the-one-that-acted.md) applied to a code action rather than a click. His rule: "click buttons when you have to, write code when you have to, and look at the result through pixels because that is the source of truth." ([Dhruv Batra](../sources/20260814_Ki980nV0__0.md), 14:16-15:23)
- **Note the direction each source arrives from.** Klein starts from pixels being expensive and adds code; Batra starts from pixels being the only surface that generalizes to the long tail and adds code as the optimization on top — "just because for the long tail you need to have a capability does not mean that is the only way you should do it." Two independent paths to the same hybrid action space, which is the strongest form of support this pattern has in the wiki. ([Dhruv Batra](../sources/20260814_Ki980nV0__0.md), 14:16-14:31)
...
- [Computer-use models will agentify the web, not APIs — Dhruv Batra, Yutori](../sources/20260814_Ki980nV0__0.md), 14:16-15:23
