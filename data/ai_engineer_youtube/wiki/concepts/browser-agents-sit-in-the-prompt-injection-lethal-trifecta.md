...
- **The lure does not have to be text.** Amazon AGI Lab's early browser-training trajectory shows a purely visual version of the same exfiltration path: the expense page carries an ad "with a submit button that looks very similar to the actual submit button" — "very common scenario that we have seen probably every day" — and the agent "enters the right amount [then] just clicks the wrong button. Now we're in a different website and it starts filling personal details." Mishra's comment is that "one can only hope that it is now hallucinating these details." No hidden instruction was needed; the untrusted-content leg of the trifecta was satisfied by page *layout*, which means text-level injection filters do not see it and the mitigation lives in [screen-perception training](train-screen-perception-primitives-beyond-coding-ability.md) and in [risk-aware action controls](keep-the-harness-thick-early-and-thin-it-as-the-model-improves.md) instead. ([From RL to IRL](../sources/20260814_Cc0_nyxROBA.md), 05:42-06:15)
...
- **The same trifecta assembled without a browser, in a go-to-market pipeline.** Private data (CRM records, product usage, meeting notes), untrusted content (a contact-sales form a prospect types into), and an external channel (outbound email through a rented sending tool) all meet inside one agent workflow. Notion's mitigation cuts the third leg rather than the second: no agent talks to a customer, and a human approves anything that leaves — which is the pattern this page implies but does not name, and which leaves the private-data-plus-untrusted-content pair intact on the read side. ([Liu](../sources/20260826_L4I7WgiEquo.md), 07:23-07:58, 13:52-14:23)
...
- [From RL to IRL — Gaurav Mishra, Amazon AGI Lab](../sources/20260814_Cc0_nyxROBA.md), 05:42-06:15
- [AI in GTM at Notion — Flora Liu](../sources/20260826_L4I7WgiEquo.md), 07:23-07:58, 13:52-14:23
